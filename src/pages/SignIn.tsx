...
export default function SignIn() {
  ...
  const pendingSource = (location.state as any)?.pendingSource || "";
  ...
  const handleEmail = async () => {
    ...
    try {
      ...
      if (pendingSource) {
        ...
        navigate("/practice", { state: { source: pendingSource } });
      } else {
        navigate("/home");
      }
    } catch (e: any) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-background">
      <div className="w-full max-w-[460px] text-center">
        ...
        <p className="text-[14px] font-sans text-ink-3 mb-10">
          Your concepts and streak are waiting.
        </p>
        ...
        {!emailMode ? (
          ...
          <div className="flex flex-col gap-3">
            <input
              ...
              className="w-full rounded-[14px] border-[1.5px] border-border bg-surface-2 px-5 py-3 text-[15px] font-sans text-foreground placeholder:text-ink-3 focus:outline-none focus:border-accent-bright transition-colors duration-[180ms]"
            />
            <input
              type="password"
              value={password}
            ...
            />
            {error && (
              <p className="text-[12px] font-sans text-destructive">{error}</p>
            )}
            <button
              onClick={handleEmail}
        ...
        <p className="text-[11px] font-sans text-ink-3 mt-6">
